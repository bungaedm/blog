...
그렇다면 이게 베이지안에게 어떤 의미를 갖는 것일까? 사전확률분포(prior model)와 가능도함수(sampling model)가 belief model `$p(y_1, ..., y_n)$`에 의존함을 의미한다. 풀어서 이야기하자면, parameter `$\theta$`가 확률론자가 주장하는 것처럼 미지의 고정된 값이 아니라, 어떤 분포를 갖는 확률변수로 볼 수 있다는 데에 아주 큰 의의가 있다. (그리고 그것을 우리는 사전확률분포 prior distribution이라고 부른다.)

그렇기 때문에 `$y_1, ..., y_n$`이 주어진
